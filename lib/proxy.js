...
module.exports = function(options) {
  ...
  function makeApiCall(req, res, next) {
    ...
    apiRequest.on('error', function(err) {
      unhandledApiError(err, apiRequestOptions, next);
    });
    ...
    apiRequest.on('response', function(resp) {
      ...
      if (_.isArray(options.transforms)) {
        apiRequest = applyTransforms(apiRequest, options.transforms, resp);
      }
  ...
  function proxyViaCache(req, res, next) {
    ...
    options.cache.exists(cacheKey, function(err, exists) {
      ...
      apiRequest.on('error', function(_err) {
        ...
        unhandledApiError(_err, apiRequestOptions, next);
      });
  ...
  function unhandledApiError(err, options, next) {
    debug('unhandled API error: %s', err.code);
    if (err.code === 'ETIMEDOUT' || err.code === 'ESOCKETTIMEDOUT') {
      return next(Error.http(504, 'API call timed out', options));
    }
    return next(err);
  }
  ...
  function applyTransforms(stream, transforms, resp) {
    // Pipe the stream through each transform in sequence
    transforms.forEach(function(transform) {
      if (transform.match && !transform.match(resp)) return;
      debug('applying transform %s', transform.name);
      if (transform.contentType) {
        resp.headers['Content-Type'] = transform.contentType;
      }
